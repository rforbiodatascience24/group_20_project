...
data_pheno <- read.csv("../data/03_metadata_aug.csv", check.names = FALSE)
...
data_counts.total <- read.csv("../data/03_count_data_wide_aug.csv", check.names = FALSE)
...
gene_symbol.df <- data_counts.total |> 
  ...
  select(gene_symbol, transcript_ID, rowname)

res <- results(dds) |> 
  as_tibble() |>
  rownames_to_column("rowname") |>
  left_join(gene_symbol.df, by = "rowname") |>
  select(-rowname)

gene_symbol.df <- gene_symbol.df |>
  select(gene_symbol)
...
write_csv(as.data.frame(res), 
          file.path("../data/DESeq2/07_DESeq2_results-global.csv"))
```
...
regulated_data <- res |>
  filter(pvalue <= 0.05) |>
  left_join(data_counts.total, by = c("transcript_ID", "gene_sy"))
```
...
## Difference in expression fasting or not in patients with non-alcoholic fatty liver disease
