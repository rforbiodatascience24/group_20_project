...
data_counts <- read.csv("../data/03_normalized_count_data_wide.csv")
...
#Saving the results
write_csv(as.data.frame(res), file.path("../data/03_DESeq2_results.csv"))
```
...
plot_volcano(x=res, title="postprandial", fc_cutoff=log2(2), p_cutoff=0.05)
```
...
plot_volcano(x=res, title="normal", fc_cutofflog2(2), 0.05)
```
