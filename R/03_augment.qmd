...
```{r}
#| label: metadata-remove-excluded
...
```{r}
#| label: normalized-count_data-pivot
...
count_data_wide <- count_data_wide |> 
  dplyr::select(c("Name",common_samples))
...
```{r}
#| label: count_data-keeping-non-null-rows
...
```{r}
#| label: count_data-renaming-columns
...
```{r}
#| label: count_data-add-gene-names
...
```{r}
#| label: count_data-renaming-new-columns
