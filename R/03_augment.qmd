...
```{r}
normalized_countdata_wide <- count_data |> 
...
normalized_countdata_wide <- normalized_countdata_wide[, c("Name",common_samples)]
```
...
normalized_countdata_wide <- normalized_countdata_wide |> 
  filter(if_all(everything(), ~ . != 0))
```
...
# all(count_data_wide$transcript_ID %in% normalized_countdata_wide$transcript_ID)
...
normalised_count_data_combined <- normalized_countdata_wide |> 
  left_join(result, by = "transcript_ID")
```
